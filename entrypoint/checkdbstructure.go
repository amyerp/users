...
func CheckDBStructure() {
	...
	if !db.Conn.Migrator().HasTable(&UserSettings{}) {
		if dbtype == "mysql" {
			db.Conn.Set("gorm:table_options", "ENGINE=InnoDB;").Migrator().CreateTable(&UserSettings{})
		} else {
			db.Conn.Migrator().CreateTable(&UserSettings{})
		}
	}

	if !db.Conn.Migrator().HasTable(&UsersInfo{}) {
		if dbtype == "mysql" {
			db.Conn.Set("gorm:table_options", "ENGINE=InnoDB;").Migrator().CreateTable(&UsersInfo{})
		} else {
			db.Conn.Migrator().CreateTable(&UsersInfo{})
		}
	}
