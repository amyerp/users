...
func enable2FA(t *pb.Request) (response *pb.Response) {

	args := ToMapStringInterface(t.Args)

	if args["code"] != nil {
		return switchon2fa(t)
	}

	return initialenable2fa(t)

}

func initialenable2fa(t *pb.Request) (response *pb.Response) {
	...
	if args["tfatype"] != nil {
		tfatype = p.Sanitize(fmt.Sprintf("%v", args["tfatype"]))
	}
...
func switchon2fa(t *pb.Request) (response *pb.Response) {
	ans := make(map[string]interface{})
	args := ToMapStringInterface(t.Args)
	p := bluemonday.UGCPolicy()
	...
	db.Conn.Table("users").Where("uid = ?", userExist.UID).Updates(map[string]interface{}{"tfa": int(1), "tfatype": tfatype})
